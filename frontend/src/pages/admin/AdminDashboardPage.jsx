...
function AdminDashboardPage() {
  ...
  const { adminAppointments, isLoadingList, error } = useAppSelector(
  ...
  const stats = useMemo(() => {
    return {
      ...
      pending: adminAppointments.filter((item) => item.status === "pending")
        .length,
      approved: adminAppointments.filter((item) => item.status === "approved")
        .length,
      rejected: adminAppointments.filter((item) => item.status === "rejected")
        .length,
    };
  }, [adminAppointments]);
  ...
  return (
    <DashboardLayout
      ...
      description="Manage patients, doctors, and appointment approvals."
    >
      <div className="space-y-6">
        ...
        <section className="grid gap-6 xl:grid-cols-3">
          <QuickActionCard
            ...
            description="View registered patients and manage block or unblock actions."
          />
          ...
          <QuickActionCard
            ...
            description="Create doctors, update fees, and manage access."
          />
          ...
          <QuickActionCard
            ...
            description="Review appointment requests and approval status."
          />
        </section>
        ...
        <section className="rounded-3xl border border-[#EEF0F6] bg-white p-6 shadow-[0_14px_38px_rgba(17,24,39,0.04)] dark:border-slate-800 dark:bg-slate-900">
          <div className="mb-5 flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-[#6B7280] dark:text-slate-400">
                Latest appointment records in the system.
              </p>
            </div>
          ...
          {isLoadingList ? (
            ...
            <div className="space-y-3">
              {adminAppointments.slice(0, 4).map((appointment) => (
                <div
                  key={appointment._id}
