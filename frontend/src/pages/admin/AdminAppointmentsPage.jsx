...
function AdminAppointmentsPage() {
  ...
  useEffect(() => {
    dispatch(
      fetchAdminAppointments(activeStatus ? { status: activeStatus } : {})
    );
  ...
  const stats = useMemo(() => {
    return {
      ...
      approved: adminAppointments.filter((item) => item.status === "approved")
        .length,
      completed: adminAppointments.filter((item) => item.status === "completed")
        .length,
        ...
        <div>
          ...
          {appointment.status === "rejected" && (
            <div className="mt-4 rounded-2xl bg-red-50 p-4">
              ...
              <p className="mt-1 line-clamp-2 text-sm font-medium text-red-700">
